...
for idx, (etiqueta, datos) in enumerate(campos_info.items()):
    ...
    entry = tk.Entry(frame_formulario, width=40 if etiqueta == "Descripción" else 25)
...
botones = ["Añadir Tarea", "Modificar Tarea", "Eliminar Tarea"]
...
lista_tareas = tk.Listbox(frame_lista, width=60, height=10)
